...
int main(int argc, char** argv) {
    ...
    s1.setString('c');
    displayTestMessage(
        "Remplacement d'une string par une autre via setString", "c", s1);

    s1 = "Retest";
    displayTestMessage(
        "Remplacement d'une string par une autre via =", "Retest", s1);

    s1 = 23;
    displayTestMessage(
        "Remplacement d'une string par une autre via = (int)", "23", s1);

    s2 = s1 + s1;
    displayTestMessage(
        "Concatenation de deux String avec + ", "2323", s2);

    s2 = s1 + s1 + "Test";
    displayTestMessage(
        "Concatenation de deux String et une chaine avec +", "2323Test", s2);

    s2 = 23;
    s2 += "Test";
    displayTestMessage("Concatenation sur une chaine avec +=", "23Test", s2);
    ...
    try {
        ...
        displayTestMessage(
            "Substring valide dans la String : s11.substring(3,3)", "DEF", s12);

        s12 = s11.substring(3, 15);
        displayTestMessage(
            "Substring trop grand dans la String : s11.substring(3,15)",
            "DEFGH",
            s12
        );
        ...
        displayTestMessage(
            "Substring invalide dans la String : s11.substring(18, 3)",
            "Erreur",
            s12
        );
